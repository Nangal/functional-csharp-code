...
namespace Exercises.Chapter5.Solutions
{
   static class Exercises
   {
      ...
      static Container<R> Map<T, R>(this Container<T> @this
         , Func<T, R> func)
         => @this.Bind(v => Container.Of(func(v)));

      static Option<R> Map<T, R>(this Option<T> @this
         , Func<T, R> func)
         => @this.Bind(v => Option.Of(func(v)));
      ...
      static IEnumerable<Tuple<Predicate<T>, Func<T, R>>> Switch<T, R>()
         => Enumerable.Empty<Tuple<Predicate<T>, Func<T, R>>>();
      
      static IEnumerable<Tuple<Predicate<T>, Func<T, R>>> Case<T, R>(
         this IEnumerable<Tuple<Predicate<T>, Func<T, R>>> rules
         , Predicate<T> pred, Func<T, R> func)
            => rules.Append(Tuple(pred, func));

      static IEnumerable<R> MatchAll<T, R>(this IEnumerable<Tuple<Predicate<T>, Func<T, R>>> rules, T t)
      ...
      static R Match<T, R>(this IEnumerable<Tuple<Predicate<T>, Func<T, R>>> rules, T t)
         => rules.Where(rule => rule.Item1(t))
            .First().Item2(t);
